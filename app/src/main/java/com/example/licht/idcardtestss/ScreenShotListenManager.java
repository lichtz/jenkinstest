...
/**
 * Created by maksim on 2018/4/28.
 */
 ...
 * <p>
 * <p>
 * 截屏判断依据: 监听媒体数据库的数据改变, 在有数据改变时获取最后 插入数据库的一条图片数据, 如果符合以下规则, 则认为截屏了: <br/>
 * <p>
...
public class ScreenShotListenManager {
    ...
    private static final String obFIlePATH = Environment.getExternalStorageDirectory() + File.separator
            + Environment.DIRECTORY_DCIM + File.separator + "Screenshots" + File.separator;

    /**
     * 截屏依据中的路径判断关键字
     */
    private static final String[] KEYWORDS = {"screenshot", "screen_shot",
            "screen-shot", "screen shot", "screencapture", "screen_capture",
            "screen-capture", "screen capture", "screencap", "screen_cap",
    ...
    public ScreenShotListenManager(Context context) {
        ...
        if (sScreenRealSize == null) {
            ...
            if (sScreenRealSize != null) {
                Log.d(TAG, "Screen Real Size: " + sScreenRealSize.x + " * "
                        + sScreenRealSize.y);
            } else {
    ...
    public void startListen() {
        ...
        mInternalObserver = new MediaContentObserver(
                MediaStore.Images.Media.INTERNAL_CONTENT_URI, mUiHandler);
        mExternalObserver = new MediaContentObserver(
                MediaStore.Images.Media.EXTERNAL_CONTENT_URI, mUiHandler);
        mCustomFileObserver = new CustomFileObserver(obFIlePATH);
    ...
    private void handleMediaContentChange(Uri contentUri) {
        ...
        try {
            ...
            cursor = mContext.getContentResolver()
                    .query(contentUri,
                            Build.VERSION.SDK_INT < 16 ? MEDIA_PROJECTIONS
            ...
            // 处理获取到的第一行数据
            boolean isSuccess = handleMediaRowData(data, dateTaken, width, height);
            //  如果不成功去检测指定路径下的文件
            handleFIleObserverData(!isSuccess);
     ...
     * 处理获取到的一行数据
     */
    private boolean handleMediaRowData(String data, long dateTaken, int width,
                                       int height) {

        if (checkScreenShot(data, dateTaken, width, height)) {
            Log.d(TAG, "ScreenShot: path = " + data + "; size = " + width
     ...
     * 判断指定的数据行是否符合截屏条件
     */
    private boolean checkScreenShot(String data, long dateTaken, int width,
                                    int height) {
        /*
         * 判断依据一: 时间判断
         */
    ...
    private static void assertInMainThread() {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            ...
            throw new IllegalStateException(
                    "Call the method must be in main thread: " + methodMsg);
        }
    }
    ...
    private class CustomFileObserver extends FileObserver {
        ...
        public void onEvent(int event, String path) {
            if (event == FileObserver.CREATE) {
                fileObserverPath = mPath+path;
            }
            // 监听到事件，做一些过滤去重处理操作
        }
    }

    private void handleFIleObserverData(boolean isSuccess) throws Exception {
        if (!isSuccess && fileObserverPath != null) {

            int[] imageWidthHeight = getImageWidthHeight(fileObserverPath);
            boolean enSure = checkFileObserverScreenShot(fileObserverPath, imageWidthHeight[0], imageWidthHeight[1]);
            if (enSure) {
                if (mListener != null && !checkCallback(fileObserverPath)) {
                    mListener.onShot(fileObserverPath);
                }
            }
            fileObserverPath = null;
        }
     ...
     * 判断指定的数据行是否符合截屏条件
     */
    private boolean checkFileObserverScreenShot(String data, int width,
                                                int height) {
